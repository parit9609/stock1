...
def train_lightgbm(config: dict, X: np.ndarray, y: np.ndarray, 
                   time_series_splits: list):
    """Train LightGBM model with time-series cross-validation"""
    try:
        logger.info("Training LightGBM model with time-series CV...")
        ...
        # Create test metrics from CV results
        if 'overall_metrics' in cv_results:
            overall = cv_results['overall_metrics']
            test_metrics = {
                'mse': overall.get('mse_mean', 0),
                'mae': overall.get('mae_mean', 0),
                'r2': overall.get('r2_mean', 0),
                'rmse': overall.get('rmse_mean', 0)
            }
        else:
            # Fallback if CV results not available
            test_metrics = {
                'mse': train_metrics['train_mse'],
                'mae': train_metrics['train_mae'],
                'r2': train_metrics['train_r2'],
                'rmse': np.sqrt(train_metrics['train_mse'])
            }
...
def train_xgboost(config: dict, X: np.ndarray, y: np.ndarray, 
                  time_series_splits: list):
    """Train XGBoost model with time-series cross-validation"""
    try:
        logger.info("Training XGBoost model with time-series CV...")
...
def train_random_forest(config: dict, X: np.ndarray, y: np.ndarray, 
                        time_series_splits: list):
    """Train Random Forest model with time-series cross-validation"""
    try:
        logger.info("Training Random Forest model with time-series CV...")
...
def train_linear_regression(config: dict, X: np.ndarray, y: np.ndarray, 
                           time_series_splits: list):
    """Train Linear Regression model with time-series cross-validation"""
    try:
        logger.info("Training Linear Regression model with time-series CV...")
...
def run_training_pipeline(config_path: str, models_to_train: list = None):
    """
    Run the complete training pipeline with time-series cross-validation
    ...
    try:
        ...
        with mlflow.start_run():
            ...
            for model_name in models_to_train:
                try:
                    logger.info(f"\n{'='*50}")
                    logger.info(f"Training {model_name.upper()} with time-series CV")
                    logger.info(f"{'='*50}")
                    ...
                    if model_name == 'lstm':
                        ...
                        # For traditional ML models, use time-series CV
                        model, train_metrics, test_metrics = available_models[model_name](
                            config, X, y, time_series_splits
                        )
            ...
            if results:
                ...
                # Print summary
                print("\n" + "="*80)
                print("TRAINING PIPELINE RESULTS SUMMARY (WITH TIME-SERIES CV)")
                print("="*80)
